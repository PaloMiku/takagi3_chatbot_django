...
#多少轮后开始执行摘要
generate_summary_num = 10

#生成摘要的命令
summary_cmd = "请总结我们的对话，要求不能超过200字"
...
def ask_openai(message, request):
    ...
    elif len(filtered_data) > generate_summary_num*2:
    ...
    user_id = str(user.id)
    if any(number in user_id for number in gpt4_userIds):
        model = str(model_4)
    else:
        model = str(model_35)
